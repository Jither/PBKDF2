...
namespace Jither.PBKDF2
{
    ...
    public class PBKDF2DeriveBytes : DeriveBytes
    {
        ...
        ///    Successive calls to GetBytes will return different results - calling
        ///    GetBytes(20) twice is  equivalent to calling GetBytes(40) once. Use 
        ...
        public override byte[] GetBytes(int keyLength)
        {
            ...
            if (buffer != null && bufferIndex > 0)
            {
                ...
                {
                    Buffer.BlockCopy(buffer, bufferIndex, result, 0, bytesFromBuffer);
                    bufferIndex += bytesFromBuffer;
                    resultIndex += bytesFromBuffer;
                }
            }

            // If, after filling from buffer, we need more bytes to fill the result,
        ...
        private void ComputeBlocks(byte[] result, int resultIndex)
        {
            ...
            while (resultIndex < result.Length)
            {
                ...
                int bytesFromBuffer = Math.Min(prf.HashSize, result.Length - resultIndex);
        ...
        private void F(int currentBlock)
        {
            ...
            {
                // Note that the PRF transform takes the immediate result of the last iteration,
                // not the combined result (in buffer):
